...
| Red List / Trophy | `CardData.redList = true` (field to add; set by keyword parsing). Any ready minion may take a `RUSH` action against a marked Red List minion as a +1 stealth directed action costing 1 blood. Trophy award timing: resolved at step 5 of the diablerie sequence (before blood hunt) — see [Combat § Diablerie](./combat.md#diablerie). | **Rule:** "A Red List minion can be marked with a master phase action. A ready vampire may enter combat with a marked Red List minion as a +1 stealth directed action costing 1 blood." |
