...
Commands that produce log entries include most state-changing commands: drawing cards, moving cards, ousting players, adjusting pool, etc. Commands that produce no log entry: `LockCard`, `UnlockCard`, `UnlockAll`, `SetCardNotes`, `SetChoice`, `SetGameNotes`.
