...
export function ChatPanel({
    ...
    return (
        <div
            className={"flex flex-col h-full min-h-0 rounded-lg border border-white/10 bg-slate-900 opacity-80 text-gray-200 overflow-hidden"}>
            {/* Header */}
            <div className="flex justify-between items-center px-4 py-2 border-b border-white/10">
                <h2 className="tracking-wide">{title}</h2>
                <div className={"flex gap-2 items-center"}>
                    <span className={"h-2 w-2 rounded-full"} style={{background: statusInfo.color}}
            ...
            {/* Messages */}
            <div
                className={"flex-1 overflow-y-auto py-2 px-3 grid grid-cols-[10rem_10rem_auto] gap-1 auto-rows-min"}>
                {messages.length === 0 && (
            ...
            <div className={"flex g-2 p-2 border-t border-white/10"}>
                ...
                />
                <button
                    className={"py-1.5 px-3 rounded-lg bg-blue-600 text-white cursor-pointer transition-opacity duration-75"}
