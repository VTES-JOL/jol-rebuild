...
interface LobbySocketContextValue {
    ...
    subscribeToGame: (gameId: string, callback: () => void) => () => void;
...
export function LobbySocketProvider({children}: {children: React.ReactNode}) {
    ...
    const gameListenersRef = useRef<Map<string, Set<() => void>>>(new Map());
    ...
    const subscribeToGame = useCallback((gameId: string, callback: () => void) => {
