...
export function CardContextMenu({card, cardRef, gameId, currentUser, playerPool, transfersRemaining, phase, isCurrentPlayer, impulseHolder, position, onCommand, onClose}: Props) {
    ...
    const hasZoneSection = showPlay || showMoveToTorpor || showRescue || showDiscard || showBurn || showInfluence || showMoveToCrypt;
        ...
        >
            ...
            {hasZoneSection && (
                <>
                    ...
                    {showMoveToCrypt && (
                        <button className={ITEM} onClick={() => fire(moveToCrypt(gameId, cardRef))}>Return to Crypt</button>
                    )}
                    {showMoveToTorpor && (
